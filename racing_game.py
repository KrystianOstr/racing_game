...
class Car:
    ...
    def move(self, keys, road_offset, road_width):
        ...
        if (
            keys[pygame.K_RIGHT]
            and self.position_x < road_offset + road_width + 20 - self.image.get_width()
        ):  # --->
            self.position_x += self.speed
...
class Obstacle:
    ...
    def move(self, screen_height, road_width, road_offset, all_obstacles):
        self.speed = self.base_speed + int(
            score / 500
        )  # Increase speed basing on score
        ...
        if self.position_y > screen_height:
            ...
            while not valid_position:
                ...
                valid_position = all(
                    abs(new_x - obstacle.position_x) > 50
    ...
    def settings_screen(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_DOWN:
                        self.current_setting = (self.current_setting + 1) % len(
...
def game_over_screen():
    ...
    while True:
        ...
        draw_text(
            screen, "GAME OVER - R to Restart, ESC to Quit", font_large, 400, 300, black
        )
